...
| `FINALIZED` | `claim_winnings` | winning position owner | `now>=claims_open_at` | positive stored allocation; not claimed | `FINALIZED` | reads only precomputed allocation; stake allocation paid; leaderboard increments after successful transfer | no participant rescan; state-before-transfer; rollback restores on failure |
| `REFUNDABLE` | `claim_refund` | position owner | `now>=claims_open_at` | positive principal; not claimed | `REFUNDABLE` | exact principal paid | bonds excluded; rollback restores |
| any terminal | `claim_creation_bond` | beneficiary | claim open | status `REFUNDABLE` | same | exact bond paid once | locked/claimed/wrong caller rejects |
| terminal or eligible post-reresolution nonterminal | `claim_challenge_bond` | challenger | status already refundable | status `REFUNDABLE` | same | exact bond paid once | added/locked/claimed rejects |
| terminal or successful-retry nonterminal | `claim_retry_bond` | retry requester | status already refundable | bonded attempt/status `REFUNDABLE` | same | exact bond paid once | none/locked/claimed rejects |

Every successful transition row projects to exactly one record under the exhaustive [architecture activity table](V4_ARCHITECTURE.md#31-write-methods). The method-to-kind mapping is closed: `create_market->MARKET_CREATED`; `stake->STAKE_ADDED`; `submit_evidence->EVIDENCE_SUBMITTED`; `close_evidence->EVIDENCE_CLOSED`; every first request and retry request `->ATTEMPT_REQUESTED`; successful execution `->ATTEMPT_SUCCEEDED`; catchably malformed execution `->ATTEMPT_FAILED`; explicit `expire_attempt->ATTEMPT_EXPIRED`; `challenge_resolution->CHALLENGE_SUBMITTED`; every branch of `finalize_market->MARKET_FINALIZED`; every reason and pool branch of `cancel_market->MARKET_CANCELLED`; and the five typed claims map respectively to `WINNINGS_CLAIMED`, `STAKE_REFUND_CLAIMED`, `CREATION_BOND_CLAIMED`, `CHALLENGE_BOND_CLAIMED`, and `RETRY_BOND_CLAIMED`. The architecture table is authoritative for the exact affected ID, previous/new phases, and received/paid values.

An atomic expiry materialization inside `retry_resolution` is an internal state effect, not a second activity: the only record is `ATTEMPT_REQUESTED` for the successor. `ATTEMPT_EXPIRED` is produced only by a successful explicit `expire_attempt`. `MARKET_FINALIZED` classifies all three successful resolution-finalization branches, including `CANCELLED` for `T=0` and `REFUNDABLE` for `T>0,W=0`; `MARKET_CANCELLED` is produced only by `cancel_market`. A rejected, reverted, or transaction-level rolled-back action produces no activity, consumes no activity ID, and for a failed transfer leaves no claim activity.
...
flowchart TD
  ...
  C --> H[Mark consumed before transfer]
  D --> H
  E --> H
  F --> H
  G --> H
  H --> I{Transfer succeeds?}
  I -->|Yes| J[Record paid; leaderboard only for winnings]
  I -->|No| K[Transaction rollback; claim remains]
...
20. `BondStatus` is closed. `CLAIMED` and `ADDED_TO_SETTLEMENT` are mutually exclusive; creation/retry bonds never use added status.
21. For each bond and aggregate: `received=paid_to_beneficiary_or_winners+locked+refundable+settlement_added_outstanding`; every transfer consumes one category exactly once. Winner consumption of added bonds is recorded per challenge.
22. At all committed states: `funds_received=funds_paid+stake_liability+locked_bond_liability+refundable_bond_liability+settlement_added_bond_liability`; fee and unattributed-surplus liabilities are zero.
23. `activity_count=len(activity_records)<=max_activity_records_per_market`; activity IDs are consecutive and consumed only on commit. Every successful public write appends exactly one record from the exhaustive architecture mapping; every rejected, reverted, transaction-level rolled-back, or transfer-failed write appends zero. Compound effects add no record: retry is only successor `ATTEMPT_REQUESTED`; all finalization branches are `MARKET_FINALIZED`; only cancellation is `MARKET_CANCELLED`; every affected ID, previous/new phase, and received/paid value matches the mapping. With `A=max_initial_attempts+max_reresolution_attempts`, configuration requires `max_activity_records_per_market >= 4 + max_stake_calls_per_market + max_evidence + 3*A + 2*max_challenges + max_positions`; this capacity proves no permitted exit can exhaust the index.
...
The stale-execution, complete-envelope execution, full-precision allocation, maximum participant/leaderboard operations, timestamp conversion, transfer/direct-value behavior, and hard-backstop parts of these invariants are mandatory GenLayer feasibility gates, not verified current capabilities. Execution/storage-dependent hard values are selected and proven during feasibility work; all ten gates must pass before production-contract coding.
