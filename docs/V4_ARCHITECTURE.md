...
- `BondStatus = NONE | LOCKED | REFUNDABLE | ADDED_TO_SETTLEMENT | CLAIMED`.
...
- `ActivityKind = MARKET_CREATED | STAKE_ADDED | EVIDENCE_SUBMITTED | EVIDENCE_CLOSED | ATTEMPT_REQUESTED | ATTEMPT_SUCCEEDED | ATTEMPT_FAILED | ATTEMPT_EXPIRED | CHALLENGE_SUBMITTED | MARKET_FINALIZED | MARKET_CANCELLED | WINNINGS_CLAIMED | STAKE_REFUND_CLAIMED | CREATION_BOND_CLAIMED | CHALLENGE_BOND_CLAIMED | RETRY_BOND_CLAIMED`.
...
| `activity_count` | `uint32` | Consecutive market-local activity records. | Increment once for every successful market write. | `max_activity_records_per_market`. | No |
| `activity_records` | `ActivityRecord[]` | Append-only queryable audit history. | Append exactly once per successful write; never delete/reorder. | Exact constructed activity cap. | No |
...
| `funds_paid_wei` | `uint256` | Successful winnings, principal, and returned-bond transfers. | Increases only after transfer success. | `<=funds_received_wei`. | No |
...
| `winning_claimed` | `bool` | Winner allocation consumed. | `false->true` before a successful transfer; rollback restores false. | Boolean. | No |
| `winning_paid_wei` | `uint256` | Successfully paid winner amount. | Zero, then exact allocation once. | `<=winning_allocation_wei`. | No |
| `stake_refund_claimed` | `bool` | Principal refund consumed. | `false->true` before transfer; rollback restores false. | Boolean. | No |
...
| `bond_status` | `BondStatus` | Current independent liability. | Section 7 transitions only. | Closed challenge subset. | No |
...
| `status` | `BondStatus` | Current consumed/liability state. | `LOCKED->REFUNDABLE->CLAIMED`; never added. | Closed enum subset. | No |
| `amount_paid_wei` | `uint256` | Returned amount. | Zero, then original amount once. | `<=original`. | No |
| `claimed_at` | `Timestamp?` | Successful claim time. | Null, then set once. | One time. | No |
...
| `kind` | `ActivityKind` | Exact successful write classification determined only by the called public method and the exhaustive branch table in section 3.1. | Immutable. | Closed enum. | No |
...
| `value_paid_wei` | `uint256` | Exact successful value paid by the write. | Immutable. | Paid funds. | No |
...
| `max_activity_records_per_market` | `uint32` | Exact append-only activity capacity. | At least the lifecycle formula below; overflow safe. | No |
...
Let `A=max_initial_attempts+max_reresolution_attempts`. The append-only activity capacity MUST satisfy the checked construction `max_activity_records_per_market >= 4 + max_stake_calls_per_market + max_evidence + 3*A + 2*max_challenges + max_positions`. The four fixed records are creation, evidence close, one terminalizing write, and the creation-bond claim. The remaining terms cover every successful stake and evidence write; at most `A` attempt-request writes; at most `A` separate execute-success, execute-failure, or explicit-expiry writes; at most `A` retry-bond claims as a safe upper bound including the two zero-bond first attempts; every challenge submission and challenge-bond claim; and at most one position claim per participant. An effective expiry materialized inside `retry_resolution` creates no extra record, so the same call consumes exactly the successor's one `ATTEMPT_REQUESTED` record. Because all source categories are independently capped and every successful public write appends exactly one record, the formula remains a conservative lifecycle bound and the activity cap cannot fill before any otherwise valid exit action. A stake at `max_stake_calls_per_market` rejects before value and mutation; critical resolution, terminalization, and claim actions MUST NOT fail for activity capacity.

## 3. Proposed public interface

All writes use a reentrancy guard where any transfer occurs. They record accounting before transfer; transaction rollback MUST restore it if transfer fails. Exactly four method categories receive value: market creation bond, stake, challenge bond, and retry bond. Every other method requires zero attached value.
...
| `claim_winnings(market_id) -> amount` | Beneficiary position owner; `FINALIZED/WINNERS`; `now>=claims_open_at`; stored `winning_allocation_wei>0`; not claimed; zero value. | Reads only the caller's precomputed allocation, marks claim/paid/statistics accounting, reduces stake/settlement liability, atomically updates the stored ranked leaderboard, appends `WINNINGS_CLAIMED`, then transfers. It never rescans market participants; leaderboard maintenance is feasibility-bounded. | Duplicate/zero allocation/wrong mode rejects. Any ranking/transfer failure rolls back the activity and all state, leaving claim retryable. |
| `claim_refund(market_id) -> amount` | Position owner; `REFUNDABLE/REFUNDS`; positive total stake; unclaimed; zero value. | Consumes exactly contributed principal, marks/refund totals, and appends `STAKE_REFUND_CLAIMED` before transfer. | **Stake only**; never claims bonds. Duplicate/zero/wrong mode rejects; transfer failure rolls back the activity and all state. |
| `claim_creation_bond(market_id) -> amount` | Exact stored beneficiary; bond status `REFUNDABLE`; terminal `FINALIZED`, `REFUNDABLE`, or `CANCELLED`; zero value. | Sets `CLAIMED`, paid amount=original, claimed time, moves liability to paid, appends `CREATION_BOND_CLAIMED`, then transfers. | `LOCKED`, `ADDED_TO_SETTLEMENT` (impossible for this type), `CLAIMED`, wrong caller reject; transfer rollback restores `REFUNDABLE` and removes the activity. |
| `claim_challenge_bond(market_id, challenge_id) -> amount` | Exact challenger; status `REFUNDABLE`; terminal or nonterminal after changed/`INVALID` successful re-resolution; zero value. | Sets `CLAIMED`, paid amount=bond, claimed time, decrements the challenger's refundable aggregate, updates liability/paid/statistics totals, appends `CHALLENGE_BOND_CLAIMED`, then transfers. | `LOCKED`, `ADDED_TO_SETTLEMENT`, `CLAIMED`, wrong ID/caller reject; transfer rollback restores every field/aggregate and removes the activity. |
| `claim_retry_bond(market_id, attempt_no) -> amount` | Exact attempt requester; bonded attempt and status `REFUNDABLE`; terminal or nonterminal after that attempt succeeds; zero value. | Sets `CLAIMED`, paid amount=bond, claimed time, decrements the requester's refundable aggregate, updates liability/paid/statistics totals, appends `RETRY_BOND_CLAIMED`, then transfers. | Zero/`NONE`, `LOCKED`, `ADDED_TO_SETTLEMENT` (impossible), `CLAIMED`, wrong caller reject; transfer rollback restores every field/aggregate and removes the activity. |

Every successful market write MUST append exactly one `ActivityRecord` to the market-local indexed `activity_records` array and increment `activity_count`; failed writes append nothing and consume no activity ID. The new `activity_id=pre-write activity_count+1`; IDs start at `1`, are consecutive, and array index `i` contains `activity_id=i+1`. Every non-null `affected_record_id` is the exact one-based evidence, attempt, or challenge ID named by its activity kind; zero is never an affected record ID. The activity-cap construction in section 2.4 proves capacity for every permitted write, including all exit actions. Event logs MAY mirror these records only after platform support is proven, but logs are not the normative audit store. Critical resolution, terminalization, and claim writes MUST NOT be blocked by activity capacity.

The following table is the exhaustive normative projection from all 17 public write methods and every activity-distinct successful branch. There is no alternative or implementation-defined activity classification.
...
| `claim_winnings` | only | `WINNINGS_CLAIMED` | `null` | `FINALIZED` | `FINALIZED` | `0` | exact successfully transferred winnings |
| `claim_refund` | only | `STAKE_REFUND_CLAIMED` | `null` | `REFUNDABLE` | `REFUNDABLE` | `0` | exact successfully transferred principal |
| `claim_creation_bond` | only | `CREATION_BOND_CLAIMED` | `null` | current phase | same phase | `0` | exact successfully returned creation bond |
| `claim_challenge_bond` | only | `CHALLENGE_BOND_CLAIMED` | challenge ID | current phase | same phase | `0` | exact successfully returned challenge bond |
| `claim_retry_bond` | only | `RETRY_BOND_CLAIMED` | attempt number | current phase | same phase | `0` | exact successfully returned retry bond |
...
The activity invariants are exact:

1. Every successful public write appends exactly one activity record.
...
10. Transfer failure rolls back the claim, all accounting and claim state, and its activity record.
...
#### 3.2.4 Complete public method signatures

The V4 ABI is exactly the following method set; `void` means no return value and no implementation-defined tuple is permitted:
...
Claims have no expiry; unclaimed liabilities remain permanently assigned. Duplicate claims reject. Leaderboard totals increase only after successful winnings transfers; refunds and returned bonds are excluded.
...
For each bond, `CLAIMED` requires prior `REFUNDABLE`; `ADDED_TO_SETTLEMENT` and `CLAIMED` are mutually exclusive. Each per-user refundable challenge/retry aggregate equals the exact sum of that beneficiary's per-bond `REFUNDABLE` records, increases once on status entry, and decreases once on successful claim. Bond amounts returned to beneficiaries or paid onward in winner claims, plus currently locked, refundable, and settlement-added outstanding amounts, equal bonds received, both per market and contract-wide.
...
At every committed observable state:
...
At winner settlement, `stake_liability=T` and `settlement_added_bond_liability=B`; assigned payouts total `T+B`. Each winner claim reduces stake-origin liability first by `min(payout, remaining stake_liability)`, then reduces settlement-added liability for the balance in ascending challenge-ID order, incrementing each challenge's `bond_settlement_paid_wei`. This attribution does not change the user's payout. There is no surplus or fee category. Production-contract coding is blocked until platform behavior proves whether unsolicited direct transfers are impossible; if possible, this revision must add an explicit nonwithdrawable quarantined-surplus category first.
...
5. outbound value-transfer rollback, reentrancy behavior, whether unsolicited or forced inbound value can change contract balance without a protocol receipt, and whether exact accounting equality is maintainable; if such inbound value is possible, this architecture must first add the nonwithdrawable quarantined-surplus treatment;
...
**Unsupported but disclosed:** current repository evidence proves V3 prompt execution and EOA transfer only. It does not prove any gate above. A failed gate blocks production-contract coding and deployment and requires explicit redesign.

## 10. Decision register

The following normative selections resolve every ABI, payable, settlement, cancellation, privilege, serialization, and bond decision. Numeric production constants remain proposed until feasibility benchmark/economic review, but no semantic alternative remains open.

| ID | Question | Recommended/selected option | Alternatives | Safety implications | Economic implications | Complexity | Status/gate |
| --- | --- | --- | --- | --- | --- | --- | --- |
| V4-D01 | Creation bond | Exact refundable configured amount; never slashed/added. | Zero; slash. | Independent one-time liability. | Spam cost but returned. | Medium. | accepted rule; value before test deployment |
| V4-D02 | Challenge bond | Exact amount; objective verdict comparison in section 7. | Always return; always slash. | Never penalized for resolver failure. | Unchanged non-invalid result can reward winners. | High. | accepted rule; value before test deployment |
...
| V4-D07 | Unclaimed funds | Perpetual assigned pull claims; no sweep. | Expiry. | No confiscation. | Dormant assigned escrow. | Medium. | accepted |
...
| V4-D20 | Audit storage | Bounded append-only market-local indexed records with constructed lifecycle capacity. | Platform events only; optional audit. | Exit actions cannot be blocked by audit exhaustion. | Predictable storage cost. | High. | accepted architecture; all storage-dependent gates before production-contract coding |
...
| V4-D22 | Read ABI | Closed bounded scalar/keyed/page DTOs in section 3.2; no storage entity or mapping return. | Storage-object returns; off-chain indexing only. | Prevents unbounded or incompatible reads. | More ABI surface. | High. | accepted; bounds proven during feasibility work before production-contract coding |
| V4-D23 | Leaderboard | Stored descending-winnings/address rank with atomic reposition; one-based rank. | First-payment order; view-time sort. | No unbounded view sort; claim update must fit. | Ranking includes only paid winnings. | High. | accepted; capacity and worst claim update proven during feasibility work before production-contract coding |
...
| V4-D25 | Claimable aggregates | Stored per-user refundable challenge/retry aggregates plus exact reconciliation. | Bounded scan in view. | Constant-layout aggregate view. | Additional writes/storage. | Medium. | accepted; storage cost proven during feasibility work before production-contract coding |
| V4-D26 | Statistics | Stored phase/receipt/payment/liability aggregates; no market scan. | Full scan; off-chain only. | Bounded exact view and reconciliation. | Additional write accounting. | Medium. | accepted; storage/update cost proven during feasibility work before production-contract coding |
