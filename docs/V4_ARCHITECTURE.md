...
Status: **proposed specification; V4 is not implemented or deployed**. `MUST`, `MUST NOT`, `SHOULD`, and `MAY` are requirements for a future V4 implementation. **Verified V3** statements are supported by the current repository. GenLayer-dependent product requirements are classified by the feasibility gates in section 9 and the [V4 release policy](../experiments/v4-gate2/V4_RELEASE_POLICY.md).
...
**Unsupported but disclosed:** the approved Gate 1 experiment proves only gate 1's two-step capability. Gate 5 and gates 2–4 and 6–10 remain unresolved. The Gate 2 forensic status is `EVIDENCE_CAPABILITY_NOT_PROVED`; it blocks forensic-proof claims and the separately authorized controlled experiment, not the product track. A failed mandatory Product Release Readiness dependency still blocks the affected production coding or deployment and requires explicit redesign. The pinned API rules are maintained in [GenLayer compatibility baseline](GENLAYER_COMPATIBILITY_BASELINE.md).
...
| V4-D13 | Two-step execution | Request then execution, contingent on product gates. | Single intelligent write. | Enables durable expiry if feasible. | Extra transaction. | High. | accepted architecture; mandatory Product Release Readiness evidence before acceptance; Gate 2 forensic proof is separately classified |
...
| V4-D16 | Serializer/digest | Exact custom canonical JSON and SHA-256 over the complete invocation envelope. | CBOR; market-data-only digest. | Consensus exact if mandatory product gates pass. | Fixed input cost. | High. | accepted architecture; mandatory Product Release Readiness dependencies before acceptance |
...
Decision gates: the non-claim normative alternatives and claim economic intent above are closed, while the Gate 5-controlled delivery ABI, storage, statuses, activity, retry/reconciliation, and accounting alternatives remain explicitly open. `AI`, `C`, `G`, `AR`, `F`, `fund_unlock_delay`, both attempt caps, maximum invocation and byte-storage caps, participant/page/activity/leaderboard/stake-call caps, and every other execution/storage-dependent hard cap MUST be selected and proven during feasibility work. Every mandatory Product Release Readiness dependency—including timing feasibility with the selected values and the product-level Gate 2 stale-write safeguards—MUST pass before its affected behavior is accepted. Complete Gate 2 protocol-forensic proof is governed separately and is not a prerequisite for beginning a V4 implementation branch. Before test deployment, implementation must be complete, remaining economic values selected, all tests pass, and an independent review report zero P0/P1/P2/P3 findings. Before production, complete the [migration acceptance stages](V4_MIGRATION_PLAN.md) and the [V4 release policy checklist](../experiments/v4-gate2/V4_RELEASE_POLICY.md#mandatory-product-release-gate), then repeat independent review with zero P0/P1/P2/P3 findings.
