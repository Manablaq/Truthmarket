...
Status: **proposed migration plan; V4 is unimplemented and undeployed**. This plan depends on the [architecture](V4_ARCHITECTURE.md), [state machine](V4_STATE_MACHINE.md), [economics and safety](V4_ECONOMICS_AND_SAFETY.md), and [test plan](V4_TEST_PLAN.md).
...
- the exact closed V4 ABI in [architecture section 3.2](V4_ARCHITECTURE.md#32-closed-bounded-read-abi), including scalar market DTOs, keyed results, bounded pages/chunks, stored/effective attempt status, caller-specific cancellation eligibility, three typed bond claims, exactly four value-receiving categories, and no fee claim/treasury method;
...
The on-chain closed `ActivityKind` enum is exactly `MARKET_CREATED | STAKE_ADDED | EVIDENCE_SUBMITTED | EVIDENCE_CLOSED | ATTEMPT_REQUESTED | ATTEMPT_SUCCEEDED | ATTEMPT_FAILED | ATTEMPT_EXPIRED | CHALLENGE_SUBMITTED | MARKET_FINALIZED | MARKET_CANCELLED | WINNINGS_CLAIMED | STAKE_REFUND_CLAIMED | CREATION_BOND_CLAIMED | CHALLENGE_BOND_CLAIMED | RETRY_BOND_CLAIMED`. The V4 ABI and frontend schema MUST implement the exhaustive [architecture write/branch mapping](V4_ARCHITECTURE.md#31-write-methods) without aliases or inferred classifications: one committed activity per successful public write, none per rejected/reverted write, retry always represented only by the successor `ATTEMPT_REQUESTED`, every `finalize_market` branch represented by `MARKET_FINALIZED`, and `MARKET_CANCELLED` reserved for `cancel_market`. The exact affected ID, previous/new phase, and received/paid values are versioned contract data and MUST NOT be reclassified by frontend migration code.

The immutable activity capacity remains the checked architecture bound `4 + max_stake_calls_per_market + max_evidence + 3*A + 2*max_challenges + max_positions`, where `A=max_initial_attempts+max_reresolution_attempts`. Deployment evidence MUST recompute this bound under the one-record rule and confirm that atomic expiry materialization inside retry consumes no extra activity record. On-chain activity IDs are one-based commit-only `pre-write activity_count+1`, and every non-null affected ID is the exact one-based evidence, attempt, or challenge ID; zero is invalid.
...
V3 and V4 leaderboards/statistics MUST remain separate by chain and contract address. V4 leaderboard values count only successfully paid V4 winnings; they exclude returned bonds, stake refunds, and V3 history. An optional aggregate view MUST label each source and MUST NOT present it as one on-chain table.

## 4. Deployment and decision stages

Progression is sequential; a failed gate stops later stages.

### Stage 0 — specification and decision acceptance

- Independent review reports zero P0/P1 specification findings.
- All semantic decisions affecting ABI, payable values, timing, bond accounting, participant indexing, settlement, cancellation, serialization, activity storage, and privileges are accepted in the specification; semantic alternatives may not remain in normative sections.
- `AI`, `C`, `G`, `AR`, `F`, `fund_unlock_delay`, both attempt caps, maximum invocation and byte-storage caps, participant/page/activity/leaderboard/stake-call caps, and every other execution/storage-dependent hard value MUST be selected and proven during the ten-gate feasibility work before production-contract coding. Purely economic bond/minimum-stake values MAY remain proposed through that work but MUST be selected and reviewed before test deployment.

### Stage 1 — isolated feasibility prototypes (no V4 production-contract coding or deployment)

Before all gates pass, work is limited to documentation, isolated throwaway prototypes, benchmarks, minimal experiments, serializer/math harnesses, and test harnesses that do not constitute the V4 production contract. Prove all ten gates in [architecture section 9](V4_ARCHITECTURE.md#9-mandatory-genlayer-feasibility-gates): two-step request/execution; stale-write protection; cancellation over pending execution; malformed-output behavior; outbound rollback, reentrancy, unsolicited/forced inbound value, and exact balance-accounting behavior; SHA-256/types plus deterministic `uint64` time conversion/storage; acceptance/finality concurrency; maximum complete invocation and exact chunk-retrievable byte storage; participant/index/full-precision allocation and ranked-leaderboard claim cost; and practical timing/attempt/pagination/statistics/view execution using the selected values. Failure requires architecture revision.

### Stage 2 — local implementation and schema tests

Only after all ten Stage 1 gates pass may a V4 production-contract implementation branch begin. Compile the future contract; run exact transition/accounting/property tests; use complete invocation-envelope/digest golden and self-contained superseded-literal negative vectors; implement the closed DTO ABI, one-based commit-only IDs and shared attempt sequence, exact single-`t_create` inclusive duration guard, processed evidence/challenge emptiness rules, first-stake participant index, precomputed allocations, stored statistics, per-user claimable aggregates, ranked leaderboard, and the exhaustive one-record-per-write activity mapping using the already selected and gate-proven timing/attempt/invocation/storage/participant/page/activity/leaderboard/stake-call values; select and review the remaining economic values; verify four and only four value-receiving categories and no owner, fee, treasury, or sweep ABI. Schema tests MUST prove zero-ID rejection, ID rollback, exact creation boundaries, the four-field empty-after-trim matrix, the effective-status five-row truth table, and every activity branch, including atomic retry and all three finalization results. No chain transaction occurs here.

### Stage 3 — isolated test deployment

Test deployment remains blocked until implementation is complete, selected parameters are benchmarked, all tests pass, and an independent review reports zero P0/P1 findings. Then deploy a new V4 address in an isolated test environment using public, recorded configuration. Verify deployed source/config/ABI/address identity. It MUST NOT replace the production V3 alias.
...
- All ten feasibility gates passed before the production-contract implementation branch began; test deployment additionally passed implementation, benchmark, test, and independent-review gates.
...
- The on-chain activity enum and exhaustive method/branch mapping are unchanged by routing: atomic retry has one successor-request record, all finalization results are `MARKET_FINALIZED`, and only cancellation is `MARKET_CANCELLED`.
