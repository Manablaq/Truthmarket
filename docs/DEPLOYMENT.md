...
- Bradbury may return transient HTML or other non-JSON gateway responses for `gen_call` / read operations such as `list_markets`. The smoke script retries these transient read failures with exponential backoff before submitting `create_market` or before continuing after a write.
