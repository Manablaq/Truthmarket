...
v3 is pending deployment and replaces v2 because the v2 resolver timed out on Bradbury due live evidence fetching inside `resolve_market`. v3 keeps strict UTC ISO deadline enforcement and AI adjudication, but resolves using only submitted evidence metadata and notes.
...
- Deploy v3 after local checks pass.
- Update the Vercel environment to the v3 contract address after deployment.
